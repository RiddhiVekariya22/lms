<template>
	<div v-if="lesson.data" class="">
		...
		<div class="gai-lesson-grid">
			<div v-if="lesson.data.no_preview" class="gai-lesson-locked border-e">
			...
			<div
				v-else
				ref="lessonContainer"
				class="gai-lesson-content bg-surface-white"
				:class="{
					...
					<div class="px-5 md:px-8 max-w-3xl">
						...
						>
							...
							<div
								...
								class="ProseMirror prose prose-table:table-fixed prose-td:p-2 prose-th:p-2 prose-td:border prose-th:border prose-td:border-outline-gray-2 prose-th:border-outline-gray-2 prose-td:relative prose-th:relative prose-th:bg-surface-gray-2 prose-sm max-w-none !whitespace-normal lesson-no-select"
								@copy.prevent
							></div>
						</div>
						<div
							v-else-if="lesson.data.instructor_notes"
							class="ProseMirror prose prose-table:table-fixed prose-td:p-2 prose-th:p-2 prose-td:border prose-th:border prose-td:border-outline-gray-2 prose-th:border-outline-gray-2 prose-td:relative prose-th:relative prose-th:bg-surface-gray-2 prose-sm max-w-none !whitespace-normal mt-8 lesson-no-select"
							@copy.prevent
						>
			...
			<div class="gai-lesson-sidebar sticky top-10">
				<div class="gai-sidebar-header">
					<div class="gai-sidebar-eyebrow">{{ __('You are studying') }}</div>
					<div class="gai-sidebar-course-title">
						{{ lesson.data.course_title }}
					</div>
					<div
						v-if="user && lesson.data.membership"
						class="gai-sidebar-progress-label"
					>
						<span>{{ Math.ceil(lessonProgress) }}%</span>
						{{ __('completed') }}
					</div>
					<ProgressBar
						v-if="user && lesson.data.membership"
...
@media (min-width: 768px) {
	.gai-lesson-grid { grid-template-columns: 70% 30%; }
}
...
.gai-sidebar-progress-label span {
	font-size: 0.9rem;
	font-weight: 700;
